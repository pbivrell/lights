...
bool reportCloud() {
  ...
  http.addHeader("Updated", updatedReport);
  ...
  updatedReport = http.header("Updated");
  ...
  for (auto c : response) {
    ...
    if (c == '\n'){
      ...
      count = 0;
      index = 0;
      LightChanged(lightStatus, id, pattern, count, updatedReport);
    }else if (c == ',') {
      if (index == 0) {
        id = item;
        ++index;
...
bool LightChanged(bool lightStatus, String ID, String pattern, uint8_t count, String updated) {
...
"    "
"    <div id=\"container\">"
"      <p class=\"subtext\">(re)configure your hub by connecting to your WiFi</p>"
"      <form action=\"/login\">"
